...
%>
  ...
  public static function idList(){
    ...
    $result = mysql_query($query) or die('Error while selecting id list.<br />'.$query);
    $idlist = array();
...
if($needrelation1Nfilter){%>
  ...
  public static function countByRelation1N($valuelist){
    ...
    $result = mysql_query($query) or die('Error while counting objects with relation 1:N filter.<br />'.$query);
    $line = mysql_fetch_array($result);
    return $line[0];
  }
...
%>
   ...
   */
  public static function getForm($submittext, <% echo $this->name; %> $postedobject=null, $action=null){
...
%>
    ...
    while($line = mysql_fetch_array($result)){
	$ret .= '      <option value="'.$line['id'].'"'.($postedobject != null && $postedobject->relation1N<% echo $field->options['relationname'];%> == $line['id'] ? ' selected="selected"' : '').'>';
<%
$first = true;
...
%>
  ...
  public static function getImageFromForm($name, $maxwidth=null, $maxheight=null){
    ...
    case IMAGETYPE_PNG:
      ...
      break;
    default:
      $errors[] = $translator->translate('generator_add_object_image_bad_type').$name;
      break;
    }
    ...
    if(count($errors) == 0 && $resource != null){
      ...
      if($status == true){
        ...
        $errors[] = $translator->translate('generator_add_object_image_resize_error').$name;
      }
    }
  }
  else{
    if(isset($_FILES[$name])){
      switch($_FILES[$name]['error']){
        case UPLOAD_ERR_INI_SIZE:
          $errors[] = $translator->translate('generator_add_object_image_too_large_file').$name;
          break;
        case UPLOAD_ERR_FORM_SIZE:
          $errors[] = $translator->translate('generator_add_object_image_html_too_large_file').$name;
          break;
        case UPLOAD_ERR_PARTIAL:
          $errors[] = $translator->translate('generator_add_object_image_partial_file').$name;
          break;
        case UPLOAD_ERR_NO_FILE:
          $errors[] = $translator->translate('generator_add_object_image_no_file').$name;
          break;
        case UPLOAD_ERR_NO_TMP_DIR:
           $errors[] = $translator->translate('generator_add_object_image_no_tmp_dir').$name;
          break;
        case UPLOAD_ERR_CANT_WRITE:
          $errors[] = $translator->translate('generator_add_object_image_no_tmp_dir').$name;
          break;
        case UPLOAD_ERR_EXTENSION:
          $errors[] = $translator->translate('generator_add_object_image_extention').$name;
          break;
        default:
          $errors[] = $translator->translate('generator_add_object_image_unknown_err').$name;
          break;
        }
	}
      else{
        $errors[] = $translator->translate('generator_add_object_image_no_such_file').$name;
      }
    }
