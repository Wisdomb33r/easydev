...
if(isset($field->options['secondobject']) && $field->options['secondobject']){%>
        ...
        global $LINK;
        $result2 = mysqli_query($LINK, $query) or die('Error while selecting N:M relations.');
...
%> WHERE id="'.$this->id.'"';
      ...
      mysqli_query($LINK, $query) or die('Error while inserting / updating object.');

      // if the object was inserted and not updated, need to retrieve its primary id
      if(! isset($this->id)){
        $query = 'SELECT LAST_INSERT_ID()';
        $result = mysqli_query($LINK, $query) or die('Cannot select last inserted id.');
...
foreach($this->fieldlist as $field){
  if($field->type == 'image'){%>
      if(isset($this-><% echo $field->label; %>_delete_flag) && $this-><% echo $field->label; %>_delete_flag === true){
        ...
        mysqli_query($LINK, 'UPDATE object_<% echo $this->name; %> SET <% echo $field->label; %>=NULL WHERE id="'.$this->id.'"') or die('Error while updating image filename.');
      }
      if(isset($this-><% echo $field->label; %>_server_temp_file) && $this-><% echo $field->label; %>_server_temp_file != ''){
        ...
        if(is_array($imageinfos) && count($imageinfos)){
          ...
          mysqli_query($LINK, 'UPDATE object_<% echo $this->name; %> SET <% echo $field->label; %>="'.$filename.'" WHERE id="'.$this->id.'"') or die('Error while updating image filename.');
        }
      }
  <%
  }
  if($field->type == 'file'){%>
      ...
      if(isset($this-><% echo $field->label; %>_server_temp_file) && $this-><% echo $field->label; %>_server_temp_file != ''){
        ...
        mysqli_query($LINK, 'UPDATE object_<% echo $this->name; %> SET <% echo $field->label; %>="'.$filepath.'" WHERE id="'.$this->id.'"') or die('Error while updating image filename.');
      }
  <%
  }
}
...
%>      $query = 'DELETE FROM object_<% echo $field->label; %>_<% echo $this->name; %>_<% echo $field->options['relationname']; %>_nmrelation '.
             'WHERE id_<% echo $this->name; %>="'.$this->id.'"';
      mysqli_query($LINK, $query) or die('Error while deleting relations N:M in second object.');
      ...
      if($this->relationNM<% echo $field->options['relationname']; %> && count($this->relationNM<% echo $field->options['relationname']; %>)){
        foreach($this->relationNM<% echo $field->options['relationname']; %> as $newlink){
          ...
          mysqli_query($LINK, $query) or die('Error while inserting relations N:M in second object.');
        }
      }
<%  }
    else{
%>      $query = 'DELETE FROM object_<% echo $this->name; %>_<% echo $field->label; %>_<% echo $field->options['relationname']; %>_nmrelation '.
             'WHERE id_<% echo $this->name; %>="'.$this->id.'"';
      mysqli_query($LINK, $query) or die('Error while deleting relations N:M in first object.');
      ...
      if($this->relationNM<% echo $field->options['relationname']; %> && count($this->relationNM<% echo $field->options['relationname']; %>)){
        foreach($this->relationNM<% echo $field->options['relationname']; %> as $newlink){
          ...
          mysqli_query($LINK, $query) or die('Error while inserting relations N:M in first object.');
        }
      }
<%  }
  }
...
foreach($this->fieldlist as $field){
  if($field->type == 'image'){%>  
     ...
     */
    public function getImage<% echo $field->label; %>Width(){
      $paddedIdentifier = str_pad($this->id, 9, '0', STR_PAD_LEFT);
      $d1 = substr($paddedIdentifier, 0, 3);
      $d2 = substr($paddedIdentifier, 3, 3);
      $directory_prefix = dirname(__FILE__).'/../';
...
foreach($this->fieldlist as $field){
  ...
  public function remove(){
    ...
    mysqli_query($LINK, $query) or die('Error while removing an object.');
...
foreach($this->fieldlist as $field){
  ...
  public static function count(){
    ...
    $result = mysqli_query($LINK, $query) or die('Error while counting objects.');
    $line = mysqli_fetch_array($result);
    return $line[0];
  }
  ...
  public static function idList(){
    ...
    $result = mysqli_query($LINK, $query) or die('Error while selecting id list.');
    $idlist = array();
  ...
  public static function find($lim = null, $nb = null){
    ...
    $result = mysqli_query($LINK, $query) or die('Error while loading objects.');
    $objectlist = array();
...
foreach($this->fieldlist as $field){
  if($field->type == 'relation1N'){%>
      ...
      self::$objects_<% echo $this->name; %>[$object->id] = $object;
    }
   ...
   */
    ...
    if(!is_numeric($id) || $id <= 0) return null;
      ...
      $result = mysqli_query($LINK, $query) or die('Error while selecting objects.');
...
if($needrelation1Nfilter){%>
  ...
  public static function countByRelation1N($valuelist){
    ...
    $result = mysqli_query($LINK, $query) or die('Error while counting objects with relation 1:N filter.');
    $line = mysqli_fetch_array($result);
    return $line[0];
  }
...
%>
    global $LINK;
    $query = '<% echo $userquery; %>';
    $result = mysqli_query($LINK, $query) or die('Error while updating objects.');
  }<%
  }
}
%>
