		...
		foreach($dbobjects as $dbobject){
			...
			foreach($dbobject->fieldlist as $field){
				if($field->type == 'image'){
					...
					if(file_exists('genscripts/object_image_'.$dbobject->name.'_'.$field->label.'.php')){
						$query = 'ROLLBACK';
						mysql_query($query) or die('Error while transaction rollback.');
						array_push($errors, Translator::translate('compile_autogenerated_page_exist_error'));
						$rolledback = true;
						...
						if(!$filepointer){
							...
							// register this field of this object as having an image script on disc (we need this information to delete it if the object is deleted)
							$query = 'INSERT INTO '.EASYDEV_OBJECTS_IMAGE_SCRIPTS.' (id_object, field_name) VALUES ("'.$mainmenuid.'", "'.$field->label.'")';
							mysql_query($query) or die('Error while image script registration.'.$query);
						}
					}
