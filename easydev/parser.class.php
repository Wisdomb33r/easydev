	  		...
	  		while($this->isTokenRelation()){
	  			...
	  			if(! $objectfound){ // if the object was not found
	  				if($relationtype == 'relationNM'){
	  					...
	  					// raise an error
	  					array_push($this->errorslist, $this->translator->translate('compile_relation_unknown_object_error').$relationname);
	  					return array();
	  				}
	  			}
	  		...
	  		while($this->isTypeToken()){
	  			...
	  			if($this->isTokenIdentifier()){
	  				...
	  				if(in_array(strtoupper($this->tokenslist[$this->currenttoken]), $mysql_reserved_tokens)){
	  					array_push($this->errorslist, $this->translator->translate('compile_mysql_reserved_token_error'));
	  					return array();
	  				}
